#!/usr/bin/env python3
"""
MACD+KDJ金叉策略 - 全市场实时监控
策略逻辑：MACD金叉 + KDJ金叉 + 放量
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('macd_kdj_monitor.log', encoding='utf-8')
    ]
)
...
class MacdKdjStrategyMonitor:
    ...
    def is_trading_time(self) -> bool:
        ...
        morning = (datetime.strptime("09:30", "%H:%M").time(), datetime.strptime("11:30", "%H:%M").time())
    ...
    def _get_full_stock_pool(self) -> List[Tuple[str, int, str]]:
        ...
        try:
            ...
            for start in range(0, min(sh_count, limit), 1000):
                ...
                if chunk:
                    for item in chunk:
                        ...
                        if code.startswith('6') and len(code) == 6 and 'ST' not in name:
            ...
            for start in range(0, min(sz_count, limit - len(stocks)), 1000):
                chunk = self.api.get_security_list(0, start)
                if chunk:
                    for item in chunk:
                        code = item['code']
                        name = item.get('name', code)
                        if (code.startswith('0') or code.startswith('3')) and len(code) == 6 and 'ST' not in name:
    ...
    def check_macd_kdj_signal(self, code: str, market: int, name: str) -> Optional[Dict]:
        """
        检查MACD+KDJ金叉信号
        条件：
        1. MACD金叉（DIF上穿DEA）
        ...
        try:
            ...
            # 条件1：MACD金叉
            macd_gold = prev['dif'] <= prev['dea'] and latest['dif'] > latest['dea']
            if not macd_gold:
                return None
            
            # 条件2：KDJ金叉
            kdj_gold = prev['k'] <= prev['d'] and latest['k'] > latest['d']
            if not kdj_gold:
                return None
            ...
            # 计算得分
            score = 70
            score += min(latest['macd'] / latest['close'] * 1000, 15)  # MACD强度
            score += min(latest['j'] / 100 * 10, 10)  # KDJ位置
            score += 5 if volume_ok else 0
            
            return {
                'code': code,
                'name': name,
                'signal_type': 'MACD+KDJ金叉',
                'score': min(score, 100),
                'price': latest['close'],
                'dif': latest['dif'],
                'dea': latest['dea'],
                ...
                'j': latest['j'],
                'reasons': [
                    f"MACD金叉 DIF={latest['dif']:.3f} DEA={latest['dea']:.3f}",
                    f"KDJ金叉 K={latest['k']:.2f} D={latest['d']:.2f}",
                    f"MACD柱状线={latest['macd']:.3f}",
                    f"放量" if volume_ok else ""
                ]
            }
        except Exception as e:
    ...
    def scan_market(self) -> List[Dict]:
        logger.info(f"\n{'='*60}")
        logger.info(f"🔍 MACD+KDJ金叉策略 - 第 {self.scan_count + 1} 次扫描")
        logger.info(f"{'='*60}")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_stock = {
                executor.submit(self.check_macd_kdj_signal, code, market, name): (code, name)
            ...
            for future in as_completed(future_to_stock):
                ...
                try:
                    ...
                    if signal and signal['score'] >= self.min_score:
                        ...
                        logger.info(f"📈 {code} {name}: MACD+KDJ金叉 (得分: {signal['score']:.0f})")
                except Exception as e:
    ...
    def run(self):
        ...
        logger.info("\n" + "="*60)
        logger.info("🚀 MACD+KDJ金叉策略监控启动")
        logger.info("="*60)
        logger.info(f"⏰ 扫描间隔: {self.scan_interval}秒 | 📊 股票: {len(self.stock_pool)} | 💰 模拟: {'开' if self.simulate_trading else '关'}")
        logger.info("="*60)
        
        self.notification_manager.notify_system_status('started', 'MACD+KDJ金叉策略监控已启动')
        ...
        try:
            ...
            self._cleanup()
            self.notification_manager.notify_system_status('stopped', 'MACD+KDJ金叉策略监控已停止')
...
if __name__ == "__main__":
    config = load_config()
    monitor = MacdKdjStrategyMonitor(config)
    try:
        monitor.run()
