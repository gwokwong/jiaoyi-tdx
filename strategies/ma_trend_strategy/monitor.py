...
"""
均线趋势策略 - 全市场实时监控
策略逻辑：MA5>MA10>MA20多头排列 + 股价在MA5之上 + MA20向上
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('ma_trend_monitor.log', encoding='utf-8')
    ]
)
...
class MATrendStrategyMonitor:
    ...
    def __init__(self, config: Dict):
        ...
        self.min_score = config.get('monitor', {}).get('min_score', 75)
    ...
    def check_ma_trend_signal(self, code: str, market: int, name: str) -> Optional[Dict]:
        """
        检查均线趋势信号
        条件：
        1. MA5 > MA10 > MA20（多头排列）
        2. 收盘价 > MA5
        3. MA20向上（当前MA20 > 昨日MA20）
        4. 成交量 > MA5均量
        """
        try:
            ...
            # 条件1：多头排列
            ma_aligned = latest['ma5'] > latest['ma10'] > latest['ma20']
            if not ma_aligned:
                return None
            ...
            # 计算得分
            score = 75
            score += min((latest['close'] - latest['ma5']) / latest['ma5'] * 100, 10)
            score += min((latest['ma5'] - latest['ma10']) / latest['ma10'] * 100 * 5, 10)
    ...
    def scan_market(self) -> List[Dict]:
        logger.info(f"\n{'='*60}")
        logger.info(f"🔍 均线趋势策略 - 第 {self.scan_count + 1} 次扫描")
        logger.info(f"{'='*60}")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_stock = {
                executor.submit(self.check_ma_trend_signal, code, market, name): (code, name)
            ...
            for future in as_completed(future_to_stock):
                ...
                try:
                    ...
                    if signal and signal['score'] >= self.min_score:
                        ...
                        logger.info(f"📈 {code} {name}: 均线趋势 (得分: {signal['score']:.0f})")
                except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            self._cleanup()
            self.notification_manager.notify_system_status('stopped', '均线趋势策略监控已停止')
...
def load_config():
    ...
    return {
        ...
        "monitor": {"scan_interval": 300, "stock_limit": 5500, "min_score": 75, "trading_hours_only": True, "max_workers": 10},
...
if __name__ == "__main__":
    config = load_config()
    monitor = MATrendStrategyMonitor(config)
    try:
        monitor.run()
