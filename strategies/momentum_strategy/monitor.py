...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('momentum_monitor.log', encoding='utf-8')
    ]
)
...
class MomentumStrategyMonitor:
    ...
    def check_momentum_signal(self, code: str, market: int, name: str) -> Optional[Dict]:
        """
        检查动量信号
        条件：
        1. 涨幅 > 3%
        ...
        try:
            ...
            # 条件3：MACD红柱
            macd_red = latest['macd'] > 0
            if not macd_red:
                return None
            
            # 条件4：突破5日高点
            break_high = latest['close'] > prev['high_5']
            if not break_high:
                return None
            
            # 计算得分
            score = 75
            score += min(change_pct * 3, 15)  # 涨幅加分
            score += min((vol_ratio - 2) * 5, 10)  # 量比加分
    ...
    def scan_market(self) -> List[Dict]:
        logger.info(f"\n{'='*60}")
        logger.info(f"🔍 动量策略 - 第 {self.scan_count + 1} 次扫描")
        logger.info(f"{'='*60}")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_stock = {
                executor.submit(self.check_momentum_signal, code, market, name): (code, name)
            ...
            for future in as_completed(future_to_stock):
                ...
                try:
                    ...
                    if signal and signal['score'] >= self.min_score:
                        ...
                        logger.info(f"📈 {code} {name}: 动量突破 (得分: {signal['score']:.0f})")
                except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            self._cleanup()
            self.notification_manager.notify_system_status('stopped', '动量策略监控已停止')
...
if __name__ == "__main__":
    config = load_config()
    monitor = MomentumStrategyMonitor(config)
    try:
        monitor.run()
